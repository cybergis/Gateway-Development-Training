    ...
    if (!Plays.findOne()) {
        ...
        for (var i=0; i<15; i++) {
            Plays.insert({
                ...
                title: "Kongfu Panda",
                schedule_id: i,
                time: new Date("2016-11-01T00:00:00").addHours(3*i),
                price: 15,
                room_id: 0,
                availNum: 400,
                rows: 20,
...
if (Meteor.isServer) {
  ...
  Api.addRoute('movies/', {
    get: function () {
        ...
        return {
            'statusCode': 200,
            'body': Plays.find({time: {$gte:now_time, $lte:end_time}},{fields: {title: 1, time: 1}, sort: { time: 1 }})
            .fetch()
        };
    }
  });
  ...
  Api.addRoute('movies/:id/schedules', {
    get: function () {
        ...
        return {
            'statusCode': 200,
            'body': Plays.find({movie_id:movie_id},{fields: {title: 1, time: 1}, sort: { time: 1 }})
            .fetch()
        };
    }
  });
  ...
  Api.addRoute('movies/:movieId/schedules/:scheduleId/rooms', {
    get: function () {
        ...
        var res = Plays.find({movie_id: movie_id, schedule_id: schedule_id},{fields: {room_title: 1, time: 1, availNum: 1}, sort: { availNum: -1 }})
        if (res.count()==0) {
            return {
                'statusCode': 400,
                'body': 'Not found'
    ...
    get: function () {
        ...
        if (sort=='available') {
            var res = Tickets.find({movie_id: movie_id, schedule_id: schedule_id, room_id: room_id, row: {$gte:row_start, $lte:row_end}},
                               {fields: {number: 1, row: 1, column: 1, available: 1},
                               sort: {available: 1}});
        }else{
            var res = Tickets.find({movie_id: movie_id, schedule_id: schedule_id, room_id: room_id, row: {$gte:row_start, $lte:row_end}},
                               {fields: {number: 1, row: 1, column: 1, available: 1},
